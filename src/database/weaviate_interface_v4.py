...
class WeaviateWCS:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.model_name_or_path == "text-embedding-ada-002":
            ...
            self.model = (
                SentenceTransformer(self.model_name_or_path)
    ...
    def show_all_collections(
        self, detailed: bool = False, max_details: bool = False
    ) -> list[str] | dict:
        """
    ...
    def _create_query_vector(self, query: str, device: str) -> list[float]:
        ...
        """
        return (
            self.get_openai_embedding(query)
            if self._openai_model
            else self.model.encode(query, device=device).tolist()
        )
    ...
    def batch_index_data(
        ...
        **kwargs,
        ...
        if not self._client.collections.exists(collection_name):
            print(
                f'Collection "{collection_name}" not found on host, creating Collection first...'
            )
            if properties is None:
                raise ValueError(
                    f"Tried to create Collection <{collection_name}> but no properties were provided."
                )
